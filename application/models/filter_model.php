...
/*INI - Modificado por: GGONZALEZ - 25/01/2015 */
    function getProducts($start_year, $end_year, $mark, $model, $start_price, $end_price, $tipo_vehiculo, $combustible, $financiamiento, $transmision, $limite, $ingreso,$recomendados, $idUsuario = "", $tipo_venta = '1') {
...
/*FIN - Modificado por: GGONZALEZ - 30/08/2016 */
        ...
        if ($mark != "") {
			...
			$sql_marca = "AND auto.marca in( ".$mark." ) ";
        ...
        if ($tipo_venta != "") {
            $sql_modelo = "AND auto.tipo_venta = ".$tipo_venta." ";
            $arregloCampos[$cont] = $tipo_venta;
            $cont ++;
        }
        ...
        if ($ingreso != "") {
            $sql_ingreso = "AND auto.tipo_ingreso = '".$ingreso."' ";
        ...
        // echo '<br>year='.$start_year.'<br/>';
         //cho '<br>year='.$end_year.'<br>';
		
		 if ($start_year != "" && $end_year != "") {
             $sql_anio = " AND auto.year BETWEEN '" . $start_year . "' AND '" . $end_year . "' ";
        } else if ($start_year != "" && $end_year == "") {
			$sql_anio = " AND auto.year >= '" . $start_year . "'";
        } else if ($end_year != "" && $start_year == "") {
            $sql_anio = " AND auto.year <= '" . $end_year . "'";
        }
        ...
        $queryToExecute = "SELECT DISTINCT (auto.id_automovil),
			...
			" . $sql_anio . "
            " . $desde_hasta . "
			  ORDER BY recomendado desc, " . $order_by . $limite;
 
		//echo nl2br($queryToExecute);
...
/*FIN - Modificado por: GGONZALEZ - 25/01/2015 */
    ...
    function paginar($total, $walls_x_pagina, $size, $objeto_fun) {
		...
		/* MODIFICADO POR GGONZALEZ-10/11/2015 INI */
		$pais = $this->session->userdata('pais');
        ...
        if ($total_paginas > $size) {
            ...
            $paginacion .= '<a href="#" onclick="' . $objeto_fun . '(\'0\',\'1\',\'\',\'\',\'' . $pais . '\')" ></a>'
                    . '<a onclick="' . $objeto_fun . '(\'' . $anterior_lapso . '\',\'' . $anterior_pagina . '\',\'\',\'\',\'' . $pais . '\') ">'
                    . '</a>';
        }
        $i = 1;
        $contador = 0;
        while ($i <= $total_paginas) {
            ...
            if ($i >= $inicio and $i <= $tope) {
                ...
                $paginacion .= '<a id="' . $i . '" '.$clase.' onclick="' . $objeto_fun . '(\'' . $contador . '\',\'' . $i . '\',\'\',\'\',\'' . $pais . '\')">   ' . $i . '  </a>';
            }
        ...
        if ($total_paginas > $size) {
            ...
            $paginacion .= '<a onclick="' . $objeto_fun . '(\'' . $siguiente_lapso . '\', \'' . $siguiente_pagina . '\',\'\',\'\',\'' . $pais . '\')"><i class="icon-angle-right"></a>
							<a onclick="' . $objeto_fun . '(\'' . $ultimo_lapso . '\', \'' . $total_paginas . '\',\'\',\'\',\'' . $pais . '\')" ></a>';
        }
        $paginacion .= '</div>';
        return $paginacion;

		    /* MODIFICADO POR GGONZALEZ-10/11/2015 FIN */
