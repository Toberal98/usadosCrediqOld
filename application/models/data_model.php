...
class Data_model extends CI_Model {
    ...
    function getCamposMatriz($anio,$pais = 1) {//para modificar
        //Top Banner
        $query = $this->db->query("SELECT id, anio, plazo, tasa, prima, recargo FROM cq_mpl_matriz_planes WHERE anio =$anio AND pais = $pais");
        //returns the banners
        return $query->row_array();
    }
    ...
    function getMarksExistentes() {
        $query = $this->db->query("SELECT DISTINCT m.id_marca as id_marca, m.nombre, m.visible 
									FROM cq_automovil a, imagenes i,  cq_marca m 
									WHERE a.estado='Aprobado' 
									AND m.id_marca= a.marca 
									AND i.codigo_vehiculo = a.id_automovil 
...
where cq_auto_x_equipamiento.id_auto = '" . $id . "')");
  ...
  /* MODIFICADO POR GGONZALEZ 13/05/2015 FIN*/
   ...
   function editUserCar($id, $pais = 1) {
	...
	function getUserPendientes($idUsuario, $estado, $limite, $placa,$pais){
		$q="SELECT auto.id_automovil AS id_auto,auto.numero_de_placa,
										...
										auto.moneda, auto.precio ,auto.year, auto.certificado, auto.kilometraje, auto.negociable,auto.estado
								FROM 
								cq_automovil AS auto 
								WHERE 
								auto.pais = '" . $pais . "'
								AND auto.estado = '" . $estado . "'";
								if($placa !=''){
									$q=$q." and auto.numero_de_placa='".$placa."' ";
								}
						$q=$q." UNION
								SELECT auto.id_automovil AS id_auto, auto.numero_de_placa,
									   marca.nombre as marca,   modelo.nombre AS modelo,  combustible.nombre tipo_comb, combustible.nombre combustible, 
										 auto.moneda, auto.precio, auto.year, auto.certificado, auto.kilometraje, auto.negociable,
										auto.estado
								FROM cq_automovil auto, cq_tipo_combustible combustible, cq_modelo modelo, cq_marca marca 
								WHERE auto.tipo_combustible = combustible.id_tipo_combustible 
									AND auto.modelo = modelo.id_modelo 
									AND auto.marca = marca.id_marca 
									AND modelo.marca = marca.id_marca 
									AND auto.estado != 'eliminado' 
									AND auto.pais = '" . $pais . "'"; 
									if($placa !=''){
										$q=$q." and auto.numero_de_placa='".$placa."' ";
									}
							$q=$q." AND auto.usuario = '".$idUsuario."' ". $limite;
			//echo $q;
			$query=$this->db->query($q);
			return $query->result_array();
	}

   function carsbystado($pais, $estado, $limite, $placa) {

       $q="SELECT DISTINCT auto.id_automovil,
					auto.year,
					auto.precio,
					auto.certificado,
								auto.recomendado,
					...
					imagenes img
					WHERE
					auto.tipo_combustible = combustible.id_tipo_combustible 
					AND auto.marca = marca.id_marca 
					AND auto.modelo = modelo.id_modelo 
					AND auto.pais = '" . $pais . "' 
					AND marca.id_marca = modelo.marca 
					AND img.codigo_vehiculo = auto.id_automovil 
					AND auto.estado = '" . $estado . "' ";
					if($placa !=''){
						$q=$q." and auto.numero_de_placa='".$placa."'";
					}
					$q=$q." order by auto.id_automovil desc " . $limite; 
 ...
 /*INI - Modificado por: GGONZALEZ - 25/01/2015*/
	...
	function getInfoCliente($id) {
        ...
        $query = $this->db->query("SELECT  fecha, ext_nombre, ext_email, ext_tel1, ext_tel2,ext_visible, justi_solicitud, ubicacion "
                . " FROM cq_automovil WHERE id_automovil = " . $id);
        return $query->result_array();
    }
	...
	function total_equipamientos() {
        $query = $this->db->query("SELECT  count(*) as total from cq_equipamiento ");
        return $query->row_array();
    }
