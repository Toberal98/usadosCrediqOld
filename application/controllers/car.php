...
class Car extends CI_Controller {
    ...
    function ver($id) {
        ...
        //cuota renting 
        $data['car']['cuota_renting'] = 0;
        $data['car']['aplica_renting'] = "no";

        $data_renting = $this->data_model->getRentingByAuto($id);
        $row_renting = $data_renting->row_array();
        
        if(!empty($row_renting)){
            $data['car']['cuota_renting'] = $row_renting['cuota_mensual'];
            $data['car']['aplica_renting'] = "si";
        }
...
//
    ...
    function administrar($action, $id) {
        ...
        //get info renting
        $data['renting'] = $this->data_model->getRentingDefault();
        $data['data_renting'] = $this->data_model->getRentingByAuto($id);
