    ...
    function savecar($action) {
      ...
      /*MODIFICADO POR GGONZALEZ 21/05/2015 -  INI*/ 
        ...
        $datos = array(
            ...
            'ext_tel2' => $ext_tel2,
            'lote' => 0,
			'recomendado' => "0" /*FIN - Modificado por: GGONZALEZ - 25/01/2015 */
        );
...
//
        ...
        for ($v = 1; $v <= 4; $v++) {
            ...
            if ($this->session->userdata('imagen' . $v) != "") {
                ...
                $archivo = $home . 'public/img_autos_temp/' . $this->session->userdata('imagen' . $v);
                ...
                if ($fp) {
                    $imgContent = fread($fp, filesize($archivo)); // cargo la imagen
                    fclose($fp);
                ...
                if ($total_imgs_tipo == 0) {
                    ...
                    $where = array('codigo_vehiculo' => $car_id,
                        'desc_tipo_foto' => $tipo_foto[$v]);

                    $this->savedata_model->update_image($datos, $where);
                }
        ...
        for ($i = 1; $i <= 20; $i++) {//Guardamos equipamiento
