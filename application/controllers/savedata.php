...
class Savedata extends CI_Controller {
    ...
    function savecar($action) {
        ...
        if ($ext_email != "") {
            ...
            if (preg_replace("^[_a-zA-Z0-9-]+(.[_a-zA-Z0-9-]+)*@+([_a-zA-Z0-9-]+.)*[a-zA-Z0-9-]{2,200}.[a-zA-Z]{2,6}$", "", $ext_email)) {
                $valida = TRUE;
            }
        }
        ...
        {
            $car_save_id = $this->savedata_model->guardar('cq_automovil', $datos); 
        }

        if($recomendado == 1){//aplica a renting
            ...
            $cargo_gps = $this->input->post('gps_r');
            $cargo_seguro = $this->input->post('seguro_r');
            ...
            $data_renting = array(
                ...
                'aplica' => "si",
                'cargo_gps' => $cargo_gps,
                'cargo_seguro' => $cargo_seguro
            );
            if($action =="update"){
                //se elimina la informacion existente del renting del auto.
                $this->db->delete('renting_auto', array('id_automovil' => $car_save_id));
            }

            $this->savedata_model->guardar('renting_auto', $data_renting); 
        }else{
            //En caso que desactive el renting
            $this->db->delete('renting_auto', array('id_automovil' => $car_save_id));
        }
