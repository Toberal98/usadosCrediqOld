...
class Site extends CI_Controller {
    ...
    function __construct() {
        ...
        $country = $this->geoip->get_country_code_by_ip($ip);

        

        if (!$this->session->userdata('pais')) {
            if ($country == 'CR') {
                $this->session->set_userdata('pais', 2);
            } elseif ($country == 'HN') {
                $this->session->set_userdata('pais', 3);
            } elseif ($country == 'SV') {
    ...
    function index($tipo_venta = '1') {
        ...
        //$consulta_sin_limite = $this->filter_model->getProducts(date('Y') - 15, date('Y'), '', '', '', '', 'todas', '', '', '', '','',0, "", $tipo_venta);
		$consulta_sin_limite = $this->filter_model->getProducts('', '', '', '', '', '', 'todas', '', '', '', '','',0, "", $tipo_venta);
        $total_resultados = count($consulta_sin_limite);

        $limite = "LIMIT " . ($desde > $total_resultados ? 0 : $desde) . ", 12";

        //Productos (Carros)
        date_default_timezone_set('America/El_Salvador');
        //Productos (Carros)
        //$data['products'] = $this->filter_model->getProducts(date('Y') - 15, date('Y'), '', '', '', '', 'todas', '', '', '', $limite,'',0, "", $tipo_venta);
		$data['products'] = $this->filter_model->getProducts('', '', '', '', '', '', 'todas', '', '', '', $limite,'',0, "", $tipo_venta);
        //Marcas
        ...
        foreach($data['products'] as $producto){
            ...
            // Creamos el objeto pr�stamo y le decimos que queremos una exactitud de 10 d�gitos despu�s de la coma.
            $prestamo = new Prestamo(10);
            // Configuramos el valor que pedimos de pr�stamo.
    ...
    function recuperarClave() {
        ...
        //$publickey = "6Le9fdQSAAAAAOd_EP6MD3RLTaua97E2oPdqhUry";
        $publickey = "6LeeeC4UAAAAAL1hTGTO1Y_47tu0lSsQbIY7Djcy";
...
;
        ...
        if (!$this->session->userdata('category')) {
            ...
            $data['products'] = $this->filter_model->getProductsVendidos($limite, $ingreso);
            //vaciamos limite para obtener la consulta de todos los autos con exactamente los mismos criterios
            $consulta_sin_limite = $this->filter_model->getProductsVendidos("", $ingreso);
            $total_resultados = count($consulta_sin_limite);
        }
        ...
        //datos de busqueda, si son cero resultados guardamos marca modelo y a�o
