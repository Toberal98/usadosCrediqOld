 ...
 $(function () {
    ...
    $(document).ready(function() {
        ...
        var colors = Highcharts.getOptions().colors,
            ...
            name = 'Browser brands',
            data = [
                    <?php foreach($topMarca as $dato){  ?>
                    {
                    y: <?php echo $dato['valor']; ?>,
                    color: colors[0],
                    drilldown: {
                                name: '<?php echo $dato['nombre']; ?>',
                                <?php 
                                    $ci = &get_instance();
                                    $ci->load->model("data_model");
     
                                    $tipo_veh=$ci->data_model->estaBytipo($dato['id_marca']); 
                                ?>
                                categories: [<?php foreach($tipo_veh as $tipo_v){ echo "'".$tipo_v['estilo']."', ";} ?>],
                                data: [<?php foreach($tipo_veh as $tipo_v){ echo $tipo_v['total'].','; } ?>],
                                color: colors[0]
                        }
                    },
        ...
        chart = new Highcharts.Chart({
            ...
            subtitle: {
                text: 'Click para ver estadisticas de tipos de vehiculo por cada marca'
            },
            xAxis: {
                categories: categories,
                labels: { rotation: 270 }
